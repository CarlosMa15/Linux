...
static void init_heap()
{
heap = mmap(NULL, heap_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | 
MAP_ANON, -1, 0);
...
{
	void *l, *prev;
	/* Check linking of doubly linked list */
	prev = NULL;
	...
	if (prev != tl)
		printf("wrong tl\n");
	/* Check that allocated links fill the expected region */
