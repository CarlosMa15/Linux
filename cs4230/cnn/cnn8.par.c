...
int main(int argc, char *argv[]) {
  ...
  double start = omp_get_wtime();
   ...
   float total = 0.0;
   #pragma omp parallel for private(n,k,c,p,q,r,s,ij,ii) reduction(+:total)
      for (n=0; n<N; n++) { // minibatch size
	for (k=0; k<K; k ++) { // output feature map
	  for (c=0; c<C; c ++) { // input feature map
	    for (p=0; p<P; p ++) { // output height
		...
		for (r=0; r<R; r ++) { // filter height
		  for (s =0; s< S; s ++) {// filter width
                    total = output_par[n][k][p][q];
		    total += input [n][c][ij+r][ii+s] * weight[k][c][r][s];
		    output_par[n][k][p][q] = total;
		  } 
		} 
	      } 
	    } 
    ...
    for (n=0; n<N; n++) { // minibatch size
      for (k=0; k<K; k ++) { // output feature map
        for (c=0; c<C; c ++) { // input feature map
	      ...
	      //ADDED FOR DEBUGGING NEEDS
	      //printf("%5s%.20f%5s%.20f\n","SEQ:",output_seq[n][k][p][q],"PAR:",output_par[n][k][p][q]);
	      //printf("%.20f\n",abs(output_seq[n][k][p][q]-output_par[n][k][p][q]));
